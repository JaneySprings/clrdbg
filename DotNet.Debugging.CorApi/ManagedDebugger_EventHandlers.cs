...
using DotNet.Debugging.CorApi.ExpressionEvaluator.Cil;
    ...
    private void HandleException(object? sender, ExceptionCorDebugManagedCallbackEventArgs exceptionEventArgs) {
        ...
        if (OnExceptionThrown is not null) {
            if (exceptionEventArgs.Unhandled)
                _exceptionPassedThroughUserCode.Remove(corThread.Id);
            OnExceptionThrown.Invoke(corThread.Id, exceptionEventArgs.Unhandled ? ExceptionStopKind.Unhandled : ExceptionStopKind.FirstChance);
            return;
        }
