...
public static class Extensions {
    ...
    public static bool HasGetter(this mdProperty mdProperty, IMetaDataImport metadataImport) {
        return metadataImport.GetPropertyProps(mdProperty).pmdGetter != 0;
    }

    public static bool MatchesVisibility(this mdFieldDef mdFieldDef, IMetaDataImport metadataImport, MemberVisibility visibility) {
        if (visibility is MemberVisibility.All) return true;
        var isPublic = mdFieldDef.IsPublic(metadataImport);
        return visibility is MemberVisibility.Public ? isPublic : isPublic is false;
    }

    public static bool MatchesVisibility(this mdProperty mdProperty, IMetaDataImport metadataImport, MemberVisibility visibility) {
        if (visibility is MemberVisibility.All) return true;
        if (mdProperty.HasGetter(metadataImport) is false) return false;
        var isPublic = mdProperty.IsPublic(metadataImport);
        return visibility is MemberVisibility.Public ? isPublic : isPublic is false;
    }
    ...
    /// </summary>
    public static bool IsDisplayable(this mdFieldDef mdFieldDef, IMetaDataImport metadataImport) {
        var fieldName = metadataImport.GetFieldProps(mdFieldDef).szField;
