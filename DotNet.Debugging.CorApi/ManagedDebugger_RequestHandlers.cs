...
using DotNet.Debugging.CorApi.ExpressionEvaluator;
    ...
    public List<(int id, string name)> GetThreads() {
        ...
        try {
            ...
            foreach (var thread in threads) {
                result.Add((thread.Id, GetThreadDisplayName(thread)));
            }
        }
        catch (Exception ex) {
        ...
        try {
            if (variablesReference.ReferenceKind is StoredReferenceKind.Scope) {
                ...
                if (variablesReference.DebuggerProxyInstance is not null) {
                    ...
                    await AddMembersAndStaticPseudoVariable(variablesReference.DebuggerProxyInstance, objectValue.ExactType, variablesReference.ThreadId, variablesReference.FrameStackDepth, result, includeNonPublicGroup: false);
                ...
                _ = await AddMembers(variablesReference.ObjectValue!, objectValue.ExactType, variablesReference.ThreadId, variablesReference.FrameStackDepth, result, MemberVisibility.NonPublic);
                SortMembers(result);
            }
                ...
                var visibility = IsUserCodeType(objectValue.ExactType) ? MemberVisibility.All : MemberVisibility.Public;
                var hasNonPublicMembers = await AddStaticMembers(variablesReference.ObjectValue!, objectValue.ExactType, variablesReference.ThreadId, variablesReference.FrameStackDepth, result, visibility);
                ...
                var objectValue = variablesReference.ObjectValue!.UnwrapDebugValueToObject();
                _ = await AddStaticMembers(variablesReference.ObjectValue!, objectValue.ExactType, variablesReference.ThreadId, variablesReference.FrameStackDepth, result, MemberVisibility.NonPublic);
                SortMembers(result);
            }
        }
        catch (Exception ex) {
