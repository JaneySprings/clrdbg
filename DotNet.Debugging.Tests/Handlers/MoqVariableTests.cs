...
public class MoqVariableTests : BaseDebugTestFixture {
    ...
    public void LazyProxyEvaluateTest() {
        ...
        // A failed evaluation is an error response, which the request helper throws for; the proxy is an object with members
        Assert.That(lazyObject.VariablesReference, Is.GreaterThan(0));
        var members = GetVariables(lazyObject.VariablesReference);
        Dump("lazy.Object", members);
        foreach (var member in members)
            Assert.That(IsFailedEvaluation(member), Is.False, member.Name);
        ...
        Assert.That(mockObject.VariablesReference, Is.GreaterThan(0));
    }

    private static bool IsFailedEvaluation(Variable variable) {
        var attributes = variable.PresentationHint?.Attributes;
        return attributes != null && attributes.Value.HasFlag(VariablePresentationHint.AttributesValue.FailedEvaluation);
    }
