...
public partial class ModuleMetadataReader : IDisposable {
    ...
    /// <summary>
    /// The managed entry point (MethodDef token) of the assembly, or null when it has none (libraries, native entry points)
    /// </summary>
    internal int? GetEntryPointMethodToken() {
        var corHeader = _peReader.PEHeaders.CorHeader;
        if (corHeader is null) return null;
        if ((corHeader.Flags & CorFlags.NativeEntryPoint) is not 0) return null;
        var entryPointToken = corHeader.EntryPointTokenOrRelativeVirtualAddress;
        // The entry point must be a MethodDef token (table 0x06)
        if (entryPointToken >> 24 is not 0x06) return null;
    ...
    public (int currentIlOffset, int? nextUserCodeIlOffset) GetFrameCurrentIlOffsetAndNextUserCodeIlOffset(ICorDebugILFrame ilFrame) {
        var method = ilFrame.GetFunction();
        var code = method.GetILCode();
        var methodToken = method.GetToken();
