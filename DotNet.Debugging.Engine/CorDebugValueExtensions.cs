...
public static class CorDebugValueExtensions {
    ...
    public static ICorDebugValue UnwrapDebugValue(this ICorDebugValue corDebugValue) {
        ...
        if (valueToCheck is ICorDebugReferenceValue refValue && !refValue.IsNull()) {
            valueToCheck = refValue.Dereference();
        }
        if (valueToCheck is ICorDebugBoxValue boxValue) {
            valueToCheck = boxValue.GetObject();
        }
    ...
    public static byte[] GetValueAsBytes(this ICorDebugGenericValue corDebugGenericValue) {
        ...
        try {
            ...
            var result = new byte[corDebugGenericValue.GetSize()];
            Marshal.Copy(buffer, result, 0, corDebugGenericValue.GetSize());
            return result;
        }
        finally {
    ...
    public static ICorDebugValue? GetClassFieldValue(this ICorDebugObjectValue objectValue, ICorDebugILFrame ilFrame, string fieldName) {
        ICorDebugType? currentType = objectValue.GetExactType();
        FieldDefToken foundFieldDef = default;
        ICorDebugClass? foundClass = null;
        IMetaDataImport? foundMetadata = null;

        // Find field on base type if necessary
        while (currentType is not null) {
            var cls = currentType.GetClass();
            var meta = cls.GetModule().GetMetaDataInterface<IMetaDataImport>();
            var field = meta.EnumFieldsWithName(cls.GetToken(), fieldName).SingleOrDefault();
            ...
            currentType = currentType.GetBase();
        }
    ...
    public static ICorDebugGenericValue GetLiteralCorDebugValue(this FieldDefToken fieldDef, IMetaDataImport metadataImport, ICorDebugILFrame ilFrame) {
        ...
        var eval = ilFrame.GetChain().GetThread().CreateEval();
    ...
    public static async Task<(ICorDebugValue Value, ICorDebugFunction? SetterFunction)?> GetPropertyValueWithSetter(this ICorDebugValue objectValue, Func<Task<CorDebugManagedCallbackEventArgs>> processEventsUntilEvalEventFunc, EvalStatus evalStatus, ICorDebugILFrame ilFrame, string propertyName) {
        ...
        ICorDebugType? currentType = unwrappedValue.GetExactType();
        PropertyToken foundPropertyDef = default;
        ICorDebugClass? foundClass = null;
        IMetaDataImport? foundMetadata = null;
        ...
        while (currentType is not null) {
            var cls = currentType.GetClass();
            var meta = cls.GetModule().GetMetaDataInterface<IMetaDataImport>();
            var prop = meta.GetPropertyWithName(cls.GetToken(), propertyName);
        ...
        if (getMethodDef == MethodDefToken.Nil) return null; // No get method
        ...
        var getMethod = foundClass.GetModule().GetFunctionFromToken(getMethodDef);
        var setMethodDef = propertyProps.pmdSetter;
        var setterFunction = setMethodDef == MethodDefToken.Nil ? null : foundClass.GetModule().GetFunctionFromToken(setMethodDef);
        var eval = ilFrame.GetChain().GetThread().CreateEval();

        // May not be correct, will need further testing
        var parameterizedContainingType = objectValue.GetExactType();
    ...
    public static async ValueTask<ICorDebugValue> GetStaticFieldValueAsync(this ICorDebugType type, Func<Task<CorDebugManagedCallbackEventArgs>> processEventsUntilEvalEventFunc, EvalStatus evalStatus, FieldDefToken fieldDef, ICorDebugILFrame ilFrame) {
        ...
        if (result is Cor.CORDBG_E_STATIC_VAR_NOT_AVAILABLE or Cor.CORDBG_E_CLASS_NOT_LOADED or Cor.E_FAIL) {
            ...
            var eval = ilFrame.GetChain().GetThread().CreateEval();
            var initializationValue = await eval.NewParameterizedObjectNoConstructorAsync(processEventsUntilEvalEventFunc, evalStatus, type.GetClass(), typeParameters.Length, typeParameters);
