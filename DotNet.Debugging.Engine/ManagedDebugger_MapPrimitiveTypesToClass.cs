...
public partial class ManagedDebugger {
    ...
    private void MapRuntimePrimitiveTypesToCorDebugClass(ICorDebugModule module) {
        if (Path.GetFileName(module.GetName()) is not "System.Private.CoreLib.dll") throw new InvalidOperationException("Mapping primitive types to classes is only supported for System.Private.CoreLib.dll");
        var metadataImport = module.GetMetaDataInterface<IMetaDataImport>();

        var typeDef = metadataImport.FindTypeDefByNameOrNull("System.Decimal", MetadataToken.Nil);
        ...
        typeDef = metadataImport.FindTypeDefByNameOrNull("System.Void", MetadataToken.Nil);
        ...
        foreach (var (corElementType, typeName) in corElementToValueNameMap) {
            var typedef = metadataImport.FindTypeDefByNameOrNull(typeName, MetadataToken.Nil);
