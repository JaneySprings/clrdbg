...
public partial class ManagedDebugger {
    private static string GetEnumDisplayValue(IMetaDataImport metaDataImport, ICorDebugObjectValue corDebugObjectValue, string valueAsString) {
        var hasFlagsAttribute = metaDataImport.TryGetCustomAttributeByName(corDebugObjectValue.GetClass().GetToken(), "System.FlagsAttribute", out _, out _) is Cor.S_OK;
    ...
    private static string? GetEnumNameForValue(IMetaDataImport metaDataImport, ICorDebugObjectValue corDebugObjectValue, string valueAsString) {
        var fields = metaDataImport.EnumFields(corDebugObjectValue.GetClass().GetToken());
    ...
    private static string GetFlagsEnumValue(IMetaDataImport metaDataImport, ICorDebugObjectValue corDebugObjectValue, string valueAsString) {
        ...
        foreach (var field in metaDataImport.EnumFields(corDebugObjectValue.GetClass().GetToken())) {
            const CorFieldAttr requiredAttributesForEnumOption = CorFieldAttr.fdPublic | CorFieldAttr.fdStatic | CorFieldAttr.fdLiteral | CorFieldAttr.fdHasDefault;
