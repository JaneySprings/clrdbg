...
internal sealed class CilValue {
    ...
    public bool IsNull => Value is null && (CorValue is null || (CorValue is ICorDebugReferenceValue refValue && refValue.IsNull()));
    ...
    /// </summary>
    public string? GetStringText() => Value as string ?? (CorValue?.UnwrapDebugValue() as ICorDebugStringValue)?.GetString();
    ...
    private bool TryReadValueTypeInteger(out long value) {
        value = 0;
        if (CorValue?.UnwrapDebugValue() is not ICorDebugGenericValue generic || generic.GetElementType() is not CorElementType.VALUETYPE) return false;
        var data = generic.GetValueAsBytes();
...
internal sealed class CorDebugLocation(ICorDebugValue value) : ICilLocation {
    ...
    public void Write(CilValue source) {
        ...
        if (destination is ICorDebugGenericValue destinationGeneric && IsValueType(destinationGeneric.GetElementType())) {
            if (source.CorValue?.UnwrapDebugValue() is ICorDebugGenericValue sourceGeneric) {
                if (destinationGeneric.GetSize() != sourceGeneric.GetSize()) throw new InvalidOperationException("CIL value sizes do not match");
            ...
            if (source.Value is not null) {
                var data = destinationGeneric.GetElementType() == CorElementType.VALUETYPE
                    ? CilValueEncoding.GetBytesForSize(source.Value, destinationGeneric.GetSize())
            ...
            if (source.IsNull) {
                var zeroed = new byte[destinationGeneric.GetSize()];
                unsafe {
    ...
    private ICorDebugValue GetStorageValue() => value is ICorDebugReferenceValue byRef && byRef.GetElementType() is CorElementType.BYREF
        ? byRef.Dereference()
        : value;
