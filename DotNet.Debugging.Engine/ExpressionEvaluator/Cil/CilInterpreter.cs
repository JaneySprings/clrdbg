...
using System.Text;
using DotNet.Debugging.Engine.ExpressionEvaluator.Compiler;
using DotNet.Debugging.CorApi;

namespace DotNet.Debugging.Engine.ExpressionEvaluator.Cil;
...
internal sealed class CilInterpreter(ManagedDebugger debugger, RuntimeAssemblyPrimitiveTypeClasses primitiveTypes) {
    public async Task<CilInterpretationResult> InterpretAsync(CompiledEvaluationMethod compiled, CompiledExpressionEvaluationContext context) {
        ...
        var (typeGenericArguments, methodGenericArguments) = context.RootValue is not null
            ? (context.RootValue.GetExactType().GetTypeParameters(), [])
        ...
        var preferredModule = context.RootValue is not null ? context.RootValue.GetExactType().GetClass().GetModule() : frame!.GetFunction().GetModule();
        var currentFrameModule = debugger.GetModuleInfoForModule(preferredModule);
        var resolver = new EvaluationMetadataResolver(debugger, compiled.MetadataReader, compiled.PeReader, context.Thread.GetAppDomain(), typeGenericArguments, methodGenericArguments, currentFrameModule);
    ...
    private static ICilLocation[] CreateArguments(ICorDebugILFrame? frame, CompiledExpressionEvaluationContext context) {
        ...
        return frame!.GetArguments().Select(value => (ICilLocation)new CorDebugLocation(value)).ToArray();
    }
    ...
    private static int GetDeclaringTypeArity(ManagedDebugger debugger, ICorDebugILFrame frame) {
        try {
            var declaringTypeToken = frame.GetFunction().GetClass().GetToken();
            var moduleInfo = debugger.GetModuleInfoForModule(frame.GetFunction().GetModule());
        ...
        while (index < instructions.Count) {
            ...
            try {
                ...
                if (op == OpCodes.Ldlen) {
                    ...
                    stack.Push(CilValue.FromPrimitive(array.GetCount()));
                    continue;
                }
                ...
                if (op == OpCodes.Call || op == OpCodes.Callvirt) {
                    ...
                    var declaringTypeArguments = !method.IsStatic && declaringTypeArity > 0 && callArguments[0].GetExactType() is { } receiverType
                        ? receiverType.GetTypeParameters().Take(declaringTypeArity).ToArray()
    ...
    private bool IsEvalAllocatableArrayElementType(ICorDebugType elementType) {
        if (elementType.GetElementType() is not CorElementType.VALUETYPE) return true;
        var token = elementType.GetClass().GetToken();
        return primitiveTypes.CorElementToValueClassMap.Values.Any(valueClass => valueClass.GetToken() == token);
    }
    ...
    private static bool IsUnboxCompatible(ICorDebugValue boxedObject, ResolvedCilType targetType) {
        ...
        if (targetType.Primitive is { } primitive) {
            ...
            return unwrapped is ICorDebugGenericValue generic && expectedElementType is not null && generic.GetElementType() == expectedElementType;
        }
        if (targetType.RuntimeType is { } runtimeType) {
            var exactType = boxedObject.GetExactType();
            if (exactType.GetElementType() is not (CorElementType.VALUETYPE or CorElementType.CLASS)) return false;
            return exactType.GetClass().GetToken() == runtimeType.Class.GetToken()
                && exactType.GetClass().GetModule().GetBaseAddress() == runtimeType.Class.GetModule().GetBaseAddress();
        }
        return false;
    }
    ...
    private async Task<CilValue> BoxAsync(CilValue value, ICorDebugType targetType, CompiledExpressionEvaluationContext context, EvaluationHandleScope handles) {
        ...
        var typeArguments = targetType.GetTypeParameters();
        var eval = context.Thread.CreateEval();
    ...
    private async Task<ICorDebugValue> MaterializeReceiverAsync(
        ...
        if (constrainedType is not null && value.CorValue?.UnwrapDebugValue() is ICorDebugGenericValue sourceGeneric) {
            ...
            var typeArguments = exactType.GetTypeParameters();
    ...
    private async Task<string> FormatInterpolatedValueAsync(
        ...
        var isDebuggeeValueType = receiver.GetExactType().GetElementType() == CorElementType.VALUETYPE;
    ...
    private static bool Compare(OpCode op, CilValue left, CilValue right) {
        if (op == OpCodes.Ceq) {
            ...
            if (left.CorValue is ICorDebugReferenceValue leftRef && right.CorValue is ICorDebugReferenceValue rightRef) return leftRef.GetValue() == rightRef.GetValue();
