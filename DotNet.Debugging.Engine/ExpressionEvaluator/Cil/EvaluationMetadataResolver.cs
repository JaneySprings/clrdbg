...
internal sealed class EvaluationMetadataResolver(ManagedDebugger debugger, MetadataReader evaluationReader, PEReader evaluationPeReader, ICorDebugAppDomain appDomain, ICorDebugType[] typeGenericArguments, ICorDebugType[] methodGenericArguments, ModuleInfo? currentFrameModule) {
    ...
    private ResolvedCilType ResolveCorDebugType(ICorDebugType type) {
        return type.GetElementType() switch {
            ...
            CorElementType.SZARRAY => new ResolvedCilType(null, null, ResolveCorDebugType(type.GetFirstTypeParameter()), 1, true),
            CorElementType.ARRAY => new ResolvedCilType(null, null, ResolveCorDebugType(type.GetFirstTypeParameter()), type.GetRank()),
            _ => throw new NotSupportedException($"Cannot resolve a CIL type from CorElementType '{type.GetElementType()}'")
        };
    }

    private ResolvedRuntimeType ResolveRuntimeType(ICorDebugType type) {
        var @class = type.GetClass();
        var moduleInfo = debugger.GetModuleInfoForModule(@class.GetModule());
        var handle = (TypeDefinitionHandle)MetadataTokens.Handle(@class.GetToken());
    ...
    private string GetTypeAssemblyName(ResolvedCilType type) {
        ...
        return reader.IsAssembly ? reader.GetString(reader.GetAssemblyDefinition().Name) : Path.GetFileNameWithoutExtension(runtimeType.Module.Module.GetName());
    }
