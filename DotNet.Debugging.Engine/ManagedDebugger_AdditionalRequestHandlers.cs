...
public partial class ManagedDebugger {
        ...
        switch (exceptionEventArgs.DwEventType) {
            ...
            case CorDebugExceptionCallbackType.DEBUG_EXCEPTION_USER_FIRST_CHANCE:
                if (exceptionEventArgs.DwEventType is CorDebugExceptionCallbackType.DEBUG_EXCEPTION_USER_FIRST_CHANCE || IsUserCodeFrame(exceptionEventArgs.Frame))
                    _exceptionPassedThroughUserCode.Add(corThread.GetId());
                break;
            case CorDebugExceptionCallbackType.DEBUG_EXCEPTION_CATCH_HANDLER_FOUND:
                var passedThroughUserCode = _exceptionPassedThroughUserCode.Remove(corThread.GetId());
                if (passedThroughUserCode && IsUserCodeFrame(exceptionEventArgs.Frame) is false && OnExceptionThrown is not null) {
                    ...
                    OnExceptionThrown.Invoke(corThread.GetId(), ExceptionStopKind.UserUnhandled);
                    return;
                }
                break;
        }
    ...
    private bool IsUserCodeFrame(ICorDebugFrame? frame) {
        try {
            ...
            return _modules.TryGetValue(ilFrame.GetFunction().GetModule().GetBaseAddress(), out var moduleInfo) && moduleInfo.IsUserCode;
        }
        catch {
            return false;
        }
    ...
    private string GetThreadDisplayName(ICorDebugThread thread) {
        ...
        try {
            ...
            _logger?.Invoke($"Failed to get the managed name of thread {thread.GetId()}: {ex.Message}");
        }

        if (thread.GetId() == _mainThreadId) return "Main Thread";

        var nativeName = ThreadNames.GetNativeThreadName(_process?.GetId() ?? 0, thread.GetId());
    ...
    private static string? GetManagedThreadName(ICorDebugThread thread) {
        if (thread.GetObject()?.UnwrapDebugValue() is not ICorDebugObjectValue threadObject) return null;

        var corClass = threadObject.GetClass();
        var metadataImport = corClass.GetModule().GetMetaDataInterface<IMetaDataImport>();
        var fieldDef = metadataImport.EnumFieldsWithName(corClass.GetToken(), "_name").SingleOrDefault();
    ...
    private void TrySetEntryPointBreakpoint(ICorDebugModule corModule, ModuleMetadataReader metadataReader) {
        ...
        try {
            ...
            var corBreakpoint = function.GetILCode().CreateBreakpoint(ilOffset);
            corBreakpoint.Activate(true);
            _entryPointBreakpoint = corBreakpoint;
            _stopAtEntryPending = false;
            _logger?.Invoke($"Entry point breakpoint set in {Path.GetFileName(corModule.GetName())} at method 0x{entryPointToken.Value:X}, IL offset {ilOffset}");
        }
        catch (Exception ex) {
    ...
    private bool TryHandleEntryPointBreakpoint(ICorDebugThread corThread, ICorDebugFunctionBreakpoint functionBreakpoint) {
        ...
        if (sourceInfo is null) OnStopped?.Invoke(corThread.GetId(), "entry");
        else OnStopped2?.Invoke(corThread.GetId(), sourceInfo.Value.FilePath, sourceInfo.Value.StartLine, sourceInfo.Value.StartColumn, "entry", null, sourceInfo.Value.DecompiledSourceInfo);
        return true;
    }
    ...
    public void SetNextStatement(int threadId, string fileName, int line) {
        ...
        if (thread.GetActiveFrame() is not ICorDebugILFrame ilFrame)
    ...
    private ICorDebugValue? FindFrameVariableValue(VariablesReference reference, string name) {
        ...
        foreach (var (index, localVariableValue) in ilFrame.GetLocalVariables().Index()) {
            var localVariableName = module.MetadataReader.GetLocalVariableName(function.GetToken(), index, currentIlOffset);
        ...
        foreach (var (index, argumentValue) in ilFrame.GetArguments().Skip(skipCount).Index()) {
            var paramDef = metadataImport.GetParamForMethodIndex(function.GetToken(), index + 1);
    ...
    private static void WriteVariableValue(ICorDebugValue target, string value) {
        ...
        var bytes = ParsePrimitiveValue(genericValue.GetElementType(), expression);
