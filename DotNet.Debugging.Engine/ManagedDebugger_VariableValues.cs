...
public partial class ManagedDebugger {
    public async Task<(string friendlyTypeName, string value, ICorDebugValue? debuggerProxyInstance, bool resultIsError)> GetValueForCorDebugValueAsync(ICorDebugValue corDebugValue, ThreadId threadId, FrameStackDepth frameStackDepth, bool escapeStringValue) {
        ArgumentNullException.ThrowIfNull(corDebugValue);
        ...
        if (debuggerProxyTypeName is not null) {
            ...
            var module = corDebugValue.GetExactType().GetClass().GetModule();
            ...
            if (!debugProxyCorDebugTypeDef.HasValue) throw new ArgumentNullException(nameof(debugProxyCorDebugTypeDef));
            ...
            var debugProxyTypeConstructorMethodDef = metadataImport.FindMethod(debugProxyCorDebugClass.GetToken(), ".ctor", 0, 0);
    ...
    private static CorDebugValueValueResult Get_CorDebugStringValue_AsString(ICorDebugStringValue corDebugStringValue, bool escapeStringValue) {
        var text = corDebugStringValue.GetString();
    ...
    public static CorDebugValueValueResult Get_CorDebugArrayValue_AsString(ICorDebugArrayValue corDebugArrayValue) {
        var typeName = GetCorDebugTypeFriendlyName(corDebugArrayValue.GetExactType());
    ...
    public static CorDebugValueValueResult GetCorDebugBoxValue_Value_AsString(ICorDebugBoxValue corDebugBoxValue, bool escapeStringValue) {
        var unboxedValue = corDebugBoxValue.GetObject();
    ...
    public static CorDebugValueValueResult GetCorDebugObjectValue_Value_AsString(ICorDebugObjectValue corDebugObjectValue, bool escapeStringValue) {
        var module = corDebugObjectValue.GetClass().GetModule();
        var metaDataImport = module.GetMetaDataInterface<IMetaDataImport>();
        var baseTypeName = corDebugObjectValue.GetExactType().GetBase() is { } baseType ? GetCorDebugTypeFriendlyName(baseType) : null; // ExactType.Base is null when ExactType is System.Object
        if (baseTypeName is "System.Enum") {
            var valueFieldDef = metaDataImport.FindField(corDebugObjectValue.GetClass().GetToken(), "value__", 0, 0);
            var valueField = corDebugObjectValue.GetFieldValue(corDebugObjectValue.GetClass(), valueFieldDef);
            ...
            return new(GetCorDebugTypeFriendlyName(corDebugObjectValue.GetExactType()), enumDisplayValue, false, null);
        }
        var typeName = GetCorDebugTypeFriendlyName(corDebugObjectValue.GetExactType());
        ...
        var hasDebuggerTypeProxyAttribute = metaDataImport.TryGetCustomAttributeByName(corDebugObjectValue.GetClass().GetToken(), "System.Diagnostics.DebuggerTypeProxyAttribute", out var debuggerTypeProxyAttributePointer, out var debuggerTypeProxyAttributeSize) is Cor.S_OK;
        var hasDebuggerDisplayAttribute = metaDataImport.TryGetCustomAttributeByName(corDebugObjectValue.GetClass().GetToken(), "System.Diagnostics.DebuggerDisplayAttribute", out var debuggerDisplayAttributePointer, out var debuggerDisplayAttributeSize) is Cor.S_OK;
        ...
        if (corDebugObjectValue.GetExactType().IsExceptionType()) {
        ...
        if (TypeOverridesToString(corDebugObjectValue.GetExactType())) {
            return new(typeName, "{ToString()}", true, debugProxyTypeName);
        }
    ...
    private static bool TypeOverridesToString(ICorDebugType corDebugType) {
        ...
        while (type is not null) {
            ...
            var module = cls.GetModule();
            var metaDataImport = module.GetMetaDataInterface<IMetaDataImport>();
            var typeName = metaDataImport.GetTypeDefProps(cls.GetToken()).szTypeDef;
            if (typeName is "System.Object" or "System.ValueType") return false;

            foreach (var methodToken in metaDataImport.EnumMethods(cls.GetToken())) {
    ...
    private static ICorDebugValue? GetUnderlyingValueOrNullFromNullableStruct(ICorDebugObjectValue corDebugObjectValue) {
        var module = corDebugObjectValue.GetClass().GetModule();
        var metaDataImport = module.GetMetaDataInterface<IMetaDataImport>();
        var hasValueFieldDef = metaDataImport.FindField(corDebugObjectValue.GetClass().GetToken(), "hasValue", 0, 0);
        var valueFieldDef = metaDataImport.FindField(corDebugObjectValue.GetClass().GetToken(), "value", 0, 0);

        var hasValueDebugObjectValue = corDebugObjectValue.GetFieldValue(corDebugObjectValue.GetClass(), hasValueFieldDef);
        var hasValueValue = GetValueForCorDebugValue(hasValueDebugObjectValue, false);
        if (hasValueValue.Value is "false") return null;
        var valueValue = corDebugObjectValue.GetFieldValue(corDebugObjectValue.GetClass(), valueFieldDef);
        return valueValue;
    }
    ...
    public static CorDebugValueValueResult GetCorDebugReferenceValue_Value_AsString(ICorDebugReferenceValue corDebugReferenceValue, bool escapeStringValue) {
        if (corDebugReferenceValue.IsNull()) {
            ...
            var typeName = GetCorDebugTypeFriendlyName(corDebugReferenceValue.GetExactType());
    ...
    internal static string GetCorDebugTypeFriendlyName(ICorDebugType corDebugType) {
        var primitiveName = GetFriendlyTypeName(corDebugType.GetElementType());
        if (primitiveName is not null) return primitiveName;
        if (corDebugType.GetElementType() is CorElementType.SZARRAY) {
            var elementName = GetCorDebugTypeFriendlyName(corDebugType.GetFirstTypeParameter());
            return $"{elementName}[]";
        }
        if (corDebugType.GetElementType() is CorElementType.ARRAY) {
            var elementName = GetCorDebugTypeFriendlyName(corDebugType.GetFirstTypeParameter());
            return $"{elementName}[{new string(',', corDebugType.GetRank() - 1)}]";
        }
        ...
        var typeParameters = corDebugType.GetTypeParameters().ToList();
    ...
    private static string GetCorDebugTypeFriendlyNameInternal(ICorDebugClass corDebugClass, List<ICorDebugType> typeParameterTypes) {
        ...
        var token = corDebugClass.GetToken();
        var metadataImport = module.GetMetaDataInterface<IMetaDataImport>();
    ...
    public static CorDebugValueValueResult GetCorDebugGenericValue_Value_AsString(ICorDebugGenericValue corDebugGenericValue) {
        IntPtr buffer = Marshal.AllocHGlobal(corDebugGenericValue.GetSize());
        try {
            ...
            var value = corDebugGenericValue.GetElementType() switch {
            ...
            var friendlyTypeName = GetFriendlyTypeName(corDebugGenericValue.GetElementType()) ?? throw new ArgumentOutOfRangeException();
