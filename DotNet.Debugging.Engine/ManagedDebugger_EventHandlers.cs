...
public partial class ManagedDebugger {
    private void HandleProcessCreated(object? sender, CreateProcessCorDebugManagedCallbackEventArgs createProcessCorDebugManagedCallbackEventArgs) {
        ...
        if (_process is null && _isRemoteAttach) {
            ...
            _isAttached = true;
            _logger?.Invoke($"Remote debuggee established connection to debugger, PID: {_process.GetId()}");
        }
        Continue();
    }
    ...
    private void HandleThreadCreated(object? sender, CreateThreadCorDebugManagedCallbackEventArgs createThreadCorDebugManagedCallbackEventArgs) {
        ...
        _mainThreadId ??= corThread.GetId();
        _threads[corThread.GetId()] = corThread;
        OnThreadStarted?.Invoke(corThread.GetId());
        Continue();
    }
    ...
    private void HandleThreadExited(object? sender, ExitThreadCorDebugManagedCallbackEventArgs exitThreadCorDebugManagedCallbackEventArgs) {
        ...
        _threads.Remove(corThread.GetId());
        OnThreadExited?.Invoke(corThread.GetId());
        Continue();
    }
    ...
    private void HandleModuleLoaded(object? sender, LoadModuleCorDebugManagedCallbackEventArgs loadModuleCorDebugManagedCallbackEventArgs) {
        ...
        var baseAddress = corModule.GetBaseAddress();
        ...
        try {
            if (corModule.IsInMemory()) {
                var size = corModule.GetSize();
                var baseAddress2 = corModule.GetBaseAddress();
        ...
        var isUserCode = corModule.GetJITCompilerFlags() is CorDebugJITCompilerFlags.CORDEBUG_JIT_DISABLE_OPTIMIZATION or CorDebugJITCompilerFlags.CORDEBUG_JIT_ENABLE_ENC;
        ...
        TrySetEntryPointBreakpoint(corModule, metadataReader);
    ...
    private async Task HandleBreakpoint(object? sender, BreakpointCorDebugManagedCallbackEventArgs breakpointCorDebugManagedCallbackEventArgs) {
        ...
        if (_asyncStepper is not null) {
            ...
            if (asyncHandled) {
                ...
                var sourceInfo = GetSourceInfoAtFrame(corThread.GetActiveFrame());
        ...
        if (TryHandleEntryPointBreakpoint(corThread, functionBreakpoint))
            return;
        ...
        if (managedBreakpoint.IsFunctionBreakpoint) {
            ...
            if (sourceInfo is null) OnStopped?.Invoke(corThread.GetId(), "breakpoint");
            else OnStopped2?.Invoke(corThread.GetId(), sourceInfo.Value.FilePath, sourceInfo.Value.StartLine, sourceInfo.Value.StartColumn, "breakpoint", [managedBreakpoint.Id], sourceInfo.Value.DecompiledSourceInfo);
            return;
        }
        ...
        OnStopped2?.Invoke(corThread.GetId(), managedBreakpoint.FilePath, resolvedBreakpoint.StartLine, resolvedBreakpoint.StartColumn, "breakpoint", [managedBreakpoint.Id], null);
    }
    ...
    private void HandleStepComplete(object? sender, StepCompleteCorDebugManagedCallbackEventArgs stepCompleteEventArgs) {
        ...
        var ilFrame = (ICorDebugILFrame)corThread.GetActiveFrame();
        ...
        _stepper = null;
        var module = _modules[ilFrame.GetFunction().GetModule().GetBaseAddress()];
        ...
        if (nextUserCodeIlOffset is null) {
            // Check attributes
            var metadataImport = ilFrame.GetFunction().GetModule().GetMetaDataInterface<IMetaDataImport>();
            var methodToken = ilFrame.GetFunction().GetToken();
            var methodIsNotDebuggable =
                metadataImport.HasAnyAttribute(methodToken, AttributeConstants.JmcMethodAttributeNames);
        ...
        OnStopped2?.Invoke(corThread.GetId(), sourceFilePath, line, column, "step", null, decompiledSourceInfo);
    }
    ...
    private void HandleBreak(object? sender, BreakCorDebugManagedCallbackEventArgs breakEventArgs) {
        ...
        OnStopped?.Invoke(corThread.GetId(), "pause");
    }
    ...
    private void HandleException(object? sender, ExceptionCorDebugManagedCallbackEventArgs exceptionEventArgs) {
        ...
        // The stop/continue decision is made by the OnExceptionThrown subscriber (exception breakpoint filters)
        if (OnExceptionThrown is not null) {
            if (exceptionEventArgs.Unhandled)
                _exceptionPassedThroughUserCode.Remove(corThread.GetId());
            OnExceptionThrown.Invoke(corThread.GetId(), exceptionEventArgs.Unhandled ? ExceptionStopKind.Unhandled : ExceptionStopKind.FirstChance);
            return;
        }
        OnStopped?.Invoke(corThread.GetId(), "exception");
    }
}