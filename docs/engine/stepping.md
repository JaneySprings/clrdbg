...
| After a hidden-finally crossing, an IP back in the statement the user's step started from (the `leave` that called the finally; the Mac Catalyst runtime ends the step there, a local runtime runs on to the next statement) | Step again with the user's kind (a step out continues as a step over): the rest of the statement remains. |
