...
| `Reflection/` | The internal Roslyn members the evaluator needs (the expression compiler's `EvaluationContext`, `MetadataBlock`, `MethodDebugInfo`, `GeneratedNameParser`, …) behind static `Internal*` wrappers over classic reflection; `RoslynReflection` resolves the assemblies and members and names a missing one. |
